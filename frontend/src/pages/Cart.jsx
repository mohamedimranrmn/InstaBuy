...
const styles = `
  ...
  .ib-cart-page * { box-sizing: border-box; }
  .ib-cart-page { font-family: 'DM Sans', sans-serif; background: #f8f7f4; min-height: 100vh; padding: 3rem 2rem; }
  ...
  .ib-cart-item-price { color: #f0a500; font-weight: 700; font-size: 0.95rem; }
  .ib-cart-item-controls { display: flex; align-items: center; gap: 0.5rem; margin-top: 0.2rem; }
  ...
  .ib-remove-btn:hover { color: #e74c3c; background: #fef2f2; }
  ...
  @media (max-width: 560px) {
    .ib-cart-item { padding: 1rem; gap: 0.875rem; }
    .ib-cart-item-img-wrap { width: 72px; height: 72px; }
    .ib-cart-item-name { font-size: 0.9rem; }
  }
`;
...
export default function Cart() {
  ...
  return (
    // ✅ FIX: renamed class from "ib-cart" to "ib-cart-page" to avoid collision
    // with the Navbar's "ib-cart" class on the cart link
    <div className="ib-cart-page">
      <style>{styles}</style>
        ...
        {cart.length === 0 ? (
          ...
          <div className="ib-cart-layout">
            ...
            <div className="ib-cart-items">
              {cart.map(item => (
                // ✅ FIX: use productId as key
                <div className="ib-cart-item" key={item.productId}>

                  {/* ✅ FIX: use imageUrl and productId for fallback */}
                  <div className="ib-cart-item-img-wrap">
                    <img
                  ...
                  {/* ✅ FIX: use productName (not item.name) */}
                  <div className="ib-cart-item-info">
                    <div className="ib-cart-item-name">{item.productName}</div>
                    <div className="ib-cart-item-price">₹{item.price?.toLocaleString("en-IN")} each</div>
                    <div className="ib-cart-item-controls">
                      {/* ✅ FIX: pass productId (not item.id) */}
                      <button className="ib-qty-btn" onClick={() => decreaseQty(item.productId)} aria-label="Decrease quantity">−</button>
                      <span className="ib-qty-val">{item.qty}</span>
                      <button className="ib-qty-btn" onClick={() => increaseQty(item.productId)} aria-label="Increase quantity">+</button>
                    </div>
                  </div>

                  {/* Right side: line total + remove */}
                  <div className="ib-cart-item-right">
                    <div className="ib-item-subtotal">
                      ₹{(item.price * item.qty).toLocaleString("en-IN")}
                    </div>
                    {/* ✅ FIX: pass productId to removeItem */}
                    <button
